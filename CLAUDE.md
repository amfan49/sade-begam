...
Flag emoji are never rendered anywhere on the site — only the country name as text. The `flag` field still exists in the data files (harmless, unused) but every renderer (`main.js`, `archive.js`, `chatbot.js`, `chat-page.js`, `api/chat.js`, `about.html`) was updated to omit it. **Western sources are always sorted before Iranian sources** wherever a list of items/sources appears (home feed, archive groups, About page source registry, chatbot/chat-page search results) — implemented as a secondary sort key: `item.country === "Iran" ? 1 : 0`.
